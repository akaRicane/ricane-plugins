...
// ─────────────────────────────────────────────────────────────────────────────
#pragma once
#include "PluginProcessor.h" // We name the NewPluginProcessor type below, so we include it.
#include <JuceHeader.h>

class NewPluginEditor final : public juce::AudioProcessorEditor {
public:
  // explicit = don't let C++ silently convert a processor into an editor.
  explicit NewPluginEditor(NewPluginProcessor&);
  ~NewPluginEditor() override = default;
...
private:
  ...
  //   auto& proc = static_cast<NewPluginProcessor&>(processor);
  ...
  JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(NewPluginEditor)
};
